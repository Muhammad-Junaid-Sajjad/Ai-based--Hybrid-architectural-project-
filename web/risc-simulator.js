...
class RISCProcessor {
    ...
    loadBenchmarkProgram() {
        ...
        // Format: [4-bit opcode][4-bit Rd][4-bit Rs1][4-bit Rs2/imm]
        //
        // Opcodes:
        // 0x0: HALT
        // 0x1: LOAD Rd, [addr]
    ...
    draw(canvas) {
        ...
        ctx.fillText('RISC Architecture', width / 2, 30);

        // Component positions
        const components = {
            pc: { x: 50, y: 80, width: 100, height: 60, label: 'PC' },
            ir: { x: 50, y: 170, width: 100, height: 60, label: 'IR' },
            regFile: { x: 250, y: 80, width: 180, height: 150, label: 'Register File' },
            alu: { x: 550, y: 120, width: 150, height: 100, label: 'ALU' },
            memory: { x: 50, y: 300, width: 200, height: 120, label: 'Memory' },
            control: { x: 550, y: 300, width: 150, height: 100, label: 'Control Unit' }
        };
        ...
        ctx.fillText(`Status: ${this.halted ? 'HALTED' : 'RUNNING'}`, 50, 485);

        // Cycle info
        ctx.fillStyle = '#333';
        ctx.font = '14px Arial';
        ctx.fillText(`Cycles: ${this.cycleCount} | Instructions: ${this.instructionCount}`, 450, 485);
    }
}
